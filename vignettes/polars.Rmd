...
Polars is a lightning fast DataFrame library/in-memory query engine. Its
embarrassingly parallel execution, cache efficient algorithms and expressive
API makes it perfect for efficient data wrangling, data pipelines, snappy APIs
and so much more.

[Polars](http://pola.rs) is among the
[fastest](https://h2oai.github.io/db-benchmark/) data table query libraries
available in any language. The underlying computation engine is written in Rust
and can be used directly from via the polars crate. Orginally Polars bindings
were implemented for Python. The **polars** R-package also provides equivalent
bindings from R. Within R, python and rust each polars package is just called
polars. The implemention polars in the different languages can be called
rust-polars, py-polars, r-polars, nodejs-polars ...


All told, **polars** users can probably expect a 5-10 speedup compared to
...
The rest of this vignette contains some simple examples to get you going. 

Users can find detailed documentation for all objects, functions, and methods [on the Reference page of this website.](https://rpolars.github.io/reference/index.html) This documentation can also be accessed from the R console using the typical `?` syntax. For example, we will later use the `DataFrame()` constructor function and apply the `groupby()` method to a `DataFrame` object. The documentation for these can be accessed by typing these commands:
...
In `polars` objects of class `Series` are analogous to vectors in R. Objects of class `DataFrame` are obviously analogous to data frames. To convert R vectors and data frames to Polars `Series` and `DataFrames`, we load the library and use constructor functions with the `pl$` prefix:
...
Although some simple R functions work out of the box on **polars** objects, the full power of Polars realized via _methods_. In this R package, Polars methods are accessed using the `$` syntax. For example:
...
Polars objects using methods than through base R functions. A second advantage
is _Methods Chaining_, a core part of the Polars workflow.

If you're coming from one of the other popular data wrangling libraries in R,
...
**polars** class.] Later on, we'll see how to read data from disk directly in
Polars format.
...
For the rest of this vignette, we'll focus on how to perform some common query tasks on DataFrames.

## Common data tasks

The `select()` method that we introduced above also supports data modification, so you can simultaneously
...
(The final `sort()` call is optional, since **polars** doesn't sort the results
