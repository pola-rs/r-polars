...
Polars supports data reshaping, going from either long to wide (`pivot`) or wide
to long (`melt`). Let's switch to the `Indometh` dataset to demonstrate via some
simple examples. Note that the data are currently in "long" format.
...
It should be noted that `pivot` can peform aggregations as part of the reshaping
operation. This is useful when you have multiple observations per ID variable
that need to be collapsed into unique values. The aggregating functions can be
arbritrarily complex, but let's consider a relatively simple example using our
`dat` ("mtcars") DataFrame from earlier: What is the median MPG value (`mpg`)
across cylinders (`cyl`), cut by different combinations of transmission type
(`am`) and engine shape (`vs`)?

```{r}
# dat$pivot(values = "mpg", index = c("am", "vs"), columns = "cyl") # errors due to non-unique obs
dat$pivot(values = "mpg", index = c("am", "vs"), columns = "cyl", aggregate_fun = "median")
```
