...
```{r, eval=purrr_parallel_available}
# Debug: step-by-step isolation of the failure
results <- list()

# Step 1: Can daemon create a polars Series?
m1 <- mirai::mirai({ polars::as_polars_series(1:3)$to_r_vector() })
r1 <- tryCatch(mirai::call_mirai(m1)$data, error = \(e) paste("ERROR:", conditionMessage(e)))
results$step1_create_series <- r1

# Step 2: Can we send a Series TO a daemon? (tests serialization)
s <- polars::as_polars_series(1:3)
m2 <- mirai::mirai(s$to_r_vector(), s = s)
r2 <- tryCatch(mirai::call_mirai(m2)$data, error = \(e) paste("ERROR:", conditionMessage(e)))
results$step2_send_series <- r2

# Step 3: Can daemon perform an operation on a Series?
m3 <- mirai::mirai((s * 2L)$to_r_vector(), s = s)
r3 <- tryCatch(mirai::call_mirai(m3)$data, error = \(e) paste("ERROR:", conditionMessage(e)))
results$step3_series_op <- r3

# Step 4: Does purrr::map with in_parallel work?
r4 <- tryCatch(
  purrr::map(list(a = s), purrr::in_parallel(\(x) x$to_r_vector())),
  error = \(e) paste("ERROR:", conditionMessage(e))
)
results$step4_in_parallel <- r4
