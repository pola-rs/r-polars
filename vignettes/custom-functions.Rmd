...
```{r, eval=purrr_parallel_available}
# Debug: test explicit register_serial call
results <- list()

s <- polars::as_polars_series(1:3)

# Step 1: Before fix — is serial registered?
r1 <- tryCatch({
  # Check via mirai's internal state
...
results$before_register <- r1

# Step 2: Explicitly call register_serial
r2 <- tryCatch({
  polars:::register_mirai_serial()
  "OK"
}, error = \(e) paste("ERROR:", conditionMessage(e)))
results$register_call <- r2

# Step 3: After register — recreate daemons to pick up config
mirai::daemons(0)
mirai::daemons(3)

# Step 4: After fix — does refhook work now?
r4 <- tryCatch({
  m <- mirai::mirai(s$to_r_vector(), s = s)
  d <- mirai::call_mirai(m)$data
  if (inherits(d, "errorValue")) paste("FAIL: errorValue", d) else paste("OK:", d)
}, error = \(e) paste("ERROR:", conditionMessage(e)))
results$after_register <- r4
