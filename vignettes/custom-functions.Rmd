...
```{r, eval=purrr_parallel_available}
# Debug: test serial_config directly with daemons()
results <- list()

s <- polars::as_polars_series(1:3)

# Step 1: Was register_serial skipped because daemons were already set?
results$daemons_set_now <- mirai::daemons_set()

# Step 2: Reset and use serial_config directly with daemons()
mirai::daemons(0)
...
results$serial_config <- if (is.list(sc)) "OK" else sc

mirai::daemons(3, serial = sc)

# Step 3: Test with serial_config passed directly
r3 <- tryCatch({
  m <- mirai::mirai(s$to_r_vector(), s = s)
  d <- mirai::call_mirai(m)$data
  if (inherits(d, "errorValue")) paste("FAIL: errorValue", d) else paste("OK:", d)
}, error = \(e) paste("ERROR:", conditionMessage(e)))
results$direct_serial <- r3

# Step 4: Also try register_serial BEFORE daemons
mirai::daemons(0)
mirai::register_serial(
  "polars_series",
  sfunc = list(\(x) x$serialize()),
  ufunc = list(\(x) polars::pl$deserialize_series(x))
)
mirai::daemons(3)
...
results$register_before_daemons <- r4
