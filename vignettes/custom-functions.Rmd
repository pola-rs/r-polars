...
```{r, eval=purrr_parallel_available}
# Debug: intentionally stop to show daemon state in error output
m <- mirai::mirai({
  list(
    lib_paths = .libPaths(),
    polars_installed = rlang::is_installed("polars"),
    polars_loaded = "polars" %in% loadedNamespaces(),
    r_libs = Sys.getenv("R_LIBS"),
    r_libs_user = Sys.getenv("R_LIBS_USER")
  )
})
daemon_state <- mirai::call_mirai(m)$data
host_state <- list(
  lib_paths = .libPaths(),
  r_libs = Sys.getenv("R_LIBS"),
  r_libs_user = Sys.getenv("R_LIBS_USER")
...
stop(paste0(
  "DEBUG DAEMON STATE: ", paste(capture.output(str(daemon_state)), collapse = "\n"),
  "\nDEBUG HOST STATE: ", paste(capture.output(str(host_state)), collapse = "\n")
))
```
