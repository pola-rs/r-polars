...
> **Important:** The version bump is critical because it triggers the `build-autogenerated-polars-version-file`
> task, which updates `DESCRIPTION`. Without this, CI pre-built binary tests will fail because
> the version in `DESCRIPTION` won't match the actual library version.
...
This runs `build-documents`, `build-standalone-files`, `build-autogenerated-files`, and then tests.
...
Use `lifecycle::deprecated()` as the default value and `lifecycle::is_present()` to detect usage.
...
Add a `lifecycle::badge("deprecated")` to the roxygen documentation and delegate to the replacement:
