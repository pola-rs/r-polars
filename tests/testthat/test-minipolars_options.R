...
test_that("set wrong options", {
  ...
  expect_error(set_minipolars_options(undefined_opt = 42 , undefined_opt_2 = 214))

  #pass a not named option
  expect_error(set_minipolars_options(42))

  #existing option but wrong type
  expect_error(set_minipolars_options(strictly_immutable = 42))

  #existing option but wrong length
  expect_error(set_minipolars_options(strictly_immutable = c(TRUE,TRUE)))

})


test_that("set/get/reset an option", {
  before_opt = get_minipolars_options()$strictly_immutable
  ...
  after_opt = set_minipolars_options(strictly_immutable= FALSE)$strictly_immutable
  ...
  default_opt = get_minipolars_options()$strictly_immutable
