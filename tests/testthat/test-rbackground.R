...
test_that("Test collecting LazyFrame in background", {
  ...
  expect_equal(res_bg$ok$to_data_frame(), compute$collect()$to_data_frame())
})
...
test_that("Test using $map() in background", {
  ...
  expect_equal(pl$get_global_rpool_cap(), pairlist(available = 0, capacity = 0))
  pl$set_global_rpool_cap(1)
  expect_equal(pl$get_global_rpool_cap(), pairlist(available = 0, capacity = 1))
