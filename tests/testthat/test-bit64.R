...
test_that("robj_to! from bit64", {
  ...
  # NO NA
  expect_error(
    unwrap(test_robj_to_i64(bit64::as.integer64(NA))),
    "scalar arguments do not support integer64 NA value"
  )
  expect_error(
    unwrap(test_robj_to_usize(bit64::as.integer64(NA))),
    "scalar arguments do not support integer64 NA value"
  )
  ...
  # NO NEGATIVE
  expect_error(
    unwrap(test_robj_to_usize(bit64::as.integer64(-1))),
    "the value -1 cannot be less than zero"
  )
  expect_error(
    unwrap(test_robj_to_u32(bit64::as.integer64(-1))),
    "the value -1 cannot be less than zero"
  )

  # NO length>1
  expect_error(
    unwrap(test_robj_to_usize(bit64::as.integer64(c(1:2)))),
