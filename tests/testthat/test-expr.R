test_that("expression boolean operators", {
  expect_s3_class(pl$col("foo") == pl$col("bar"), "Expr")
  expect_s3_class(pl$col("foo") >= pl$col("bar"), "Expr")
  expect_s3_class(pl$col("foo") <= pl$col("bar"), "Expr")
  expect_s3_class(pl$col("foo") != pl$col("bar"), "Expr")

  expect_s3_class(pl$col("foo") > pl$lit(5), "Expr")
  expect_s3_class(pl$col("foo") < pl$lit(5), "Expr")
  expect_s3_class(pl$col("foo") > 5, "Expr")
  expect_s3_class(pl$col("foo") < 5, "Expr")
  expect_s3_class(!pl$col("foobar"), "Expr")
...
test_that("and or is_in xor", {
  df <- pl$DataFrame(list())
  expect_true(df$select(pl$lit(TRUE) & TRUE)$as_data_frame()[[1L]])
  expect_true(!df$select(pl$lit(TRUE) & FALSE)$as_data_frame()[[1L]])
  expect_true(!df$select(pl$lit(FALSE) & TRUE)$as_data_frame()[[1L]])
  expect_true(!df$select(pl$lit(FALSE) & FALSE)$as_data_frame()[[1L]])
  ...
  expect_true(!df$select(pl$lit(TRUE)$xor(pl$lit(TRUE)))$as_data_frame()[[1L]])
  expect_true(df$select(pl$lit(TRUE)$xor(pl$lit(FALSE)))$as_data_frame()[[1L]])
  expect_true(df$select(pl$lit(FALSE)$xor(pl$lit(TRUE)))$as_data_frame()[[1L]])
  expect_true(!df$select(pl$lit(FALSE)$xor(pl$lit(FALSE)))$as_data_frame()[[1L]])
...
test_that("to_physical + cast", {
  ...
  expect_true(
    df_big_n$
      with_columns(
        pl$col("big")$cast(pl$Int32, strict = FALSE)$is_null()
      )$
      as_data_frame()$
      big
  )
  ...
  expect_false(
    df_big_n$
      with_columns(
        pl$col("big")$cast(pl$Int64)$is_null()
      )$
      as_data_frame()$
      big
  )
...
test_that("finite infinte is_nan is_not_nan", {
  ...
  expect_identical(
    ...
    list(
      ...
      is_not_nan  = c(TRUE, FALSE, TRUE, TRUE, TRUE)
    )
  )
})
...
test_that("sort_by", {
  ...
  expect_error(pl$lit(1:4)$sort_by(1)$to_r(), "Expected length\\: 4")
  expect_error(pl$lit(1:4)$sort_by("blop")$to_r(), "column 'blop' not available in schema")
  expect_error(pl$lit(1:4)$sort_by("blop")$to_r(), "column 'blop' not available in schema")
  expect_error(pl$lit(1:4)$sort_by(df)$to_r(), "not convertable into.* Expr")
  expect_error(pl$lit(1:4)$sort_by(df)$to_r(), "not convertable into.* Expr")
...
test_that("std var", {
  ...
  expect_different(
    pl$select(pl$lit(1:5)$std(3))$to_list()[[1L]],
    sd(1:5)
  )
  ...
  expect_different(
    pl$select(pl$lit(1:5)$var(3))$to_list()[[1L]],
    var(1:5)
  )

  # trigger u8 conversion errors
  expect_error(pl$lit(1:321)$std(256), "ddof | exceeds u8 max value")
  expect_error(pl$lit(1:321)$var(-1), "ddof | the value -1 cannot be less than zero")
})
...
test_that("is_between", {
  ...
  expect_identical(
    df$select(pl$col("a")$is_between(2, 4, TRUE))$to_list()[[1L]],
    c(FALSE, TRUE, TRUE, TRUE, FALSE)
  )
  expect_identical(
    df$select(pl$col("a")$is_between(2, 4, c(TRUE, FALSE)))$to_list()[[1L]],
    c(FALSE, TRUE, TRUE, FALSE, FALSE)
  )
  expect_identical(
    df$select(pl$col("a")$is_between(2, 4, c(FALSE, TRUE)))$to_list()[[1L]],
    c(FALSE, FALSE, TRUE, TRUE, FALSE)
  )
  expect_identical(
    df$select(pl$col("a")$is_between(1.9, 4.1, FALSE))$to_list()[[1L]],
    c(FALSE, TRUE, TRUE, TRUE, FALSE)
  )
...
test_that("hash + reinterpret", {
  ...
  expect_different(df_hash$to_list(), df_hash_rein$to_list())
...
test_that("inspect", {
  ...
  expect_no_error(pl$lit(1)$inspect("{}"))
  expect_no_error(pl$lit(1)$inspect("ssdds{}sdsfsd"))
...
test_that("Expr_diff", {
  ...
  expect_no_error(pl$select(pl$lit(1:5)$diff(0)))
  expect_error(
    pl$lit(1:5)$diff(-1),
    "less than zero"
  )
...
test_that("rep_extend", {
  ...
  expect_error(
    pl$lit(1)$rep_extend(1, -1),
    "less than zero"
  )
  expect_error(
    pl$lit(1)$rep_extend(1, Inf),
    "exceeds"
  )
})
