...
test_that("$then() works if there's no $otherwise() condition", {
  test = pl$DataFrame(x = 1:4)
  # select
  expect_identical(
    test$select(
      pl$when(pl$col("x") <= 2)$then(pl$lit("lower"))
    )$to_list(),
    list(literal = c("lower", "lower", NA, NA))
  )
  # select with alias
  expect_identical(
    test$select(
      pl$when(pl$col("x") <= 2)$then(pl$lit("lower"))$alias("foo")
    )$to_list(),
    list(foo = c("lower", "lower", NA, NA))
  )
  # with_columns
  expect_identical(
    test$with_columns(
    ...
    list(x = 1:4, literal = c("lower", "lower", NA, NA))
  )
  # with_columns with alias
  expect_identical(
    test$with_columns(
      pl$when(pl$col("x") <= 2)$then(pl$lit("lower"))$alias("foo")
    )$to_list(),
    list(x = 1:4, foo = c("lower", "lower", NA, NA))
  )
})

test_that("chained $then() works if there's no $otherwise() condition", {
  test = pl$DataFrame(x = 1:4)
  # select
  expect_identical(
    test$select(
      pl$when(pl$col("x") <= 2)$then(pl$lit("lower"))$
        when(pl$col("x") > 10)$then(pl$lit("foo"))
    )$to_list(),
    list(literal = c("lower", "lower", NA, NA))
  )
  # select with alias
  expect_identical(
    test$select(
      pl$when(pl$col("x") <= 2)$then(pl$lit("lower"))$
        when(pl$col("x") > 10)$then(pl$lit("foo"))$alias("foo")
    )$to_list(),
    list(foo = c("lower", "lower", NA, NA))
  )
  # with_columns
