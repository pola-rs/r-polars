test_that("when", {
  ...
  expect_error(pl$when(complex(2)), c("in pl\\$when", "predicate", "not convertable into Expr"))
...
test_that("whenthenthen", {
  ...
  expect_error(wtt$when(complex(1)), c("in WhenThenThen\\$when", "predicate", "into Expr"))
  expect_error(wtt$then(complex(1)), c("in WhenThenThen\\$then", "expr", "into Expr"))
  expect_error(
