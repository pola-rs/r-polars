...
readr::write_csv(test, "data/test.csv")
```
...
end. We can now compare the two approaches:
...
So what happened? Under the hood, `polars` reorganized the query so that it filters
the data first, and then sorts the remaining data. This can be seen by comparing 
the original query (`describe_plan()`) and the optimized query (`describe_optimized_plan()`):
