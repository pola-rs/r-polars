...
ExprBin_starts_with = function(sub) {
    .pr$Expr$bin_starts_with(self, sub)
}
...
ExprBin_ends_with = function(sub) {
    .pr$Expr$bin_ends_with(self, sub)
}
...
ExprBin_encode = function(encoding) {
    if (identical(encoding, "hex"))
        .pr$Expr$bin_encode_hex(self)
    else if (identical(encoding, "base64"))
        .pr$Expr$bin_encode_base64(self)
    else
        stop("encoding must be one of {{'hex', 'base64'}}", call. = FALSE)
}
...
ExprBin_decode = function(encoding, strict = TRUE) {
    if (identical(encoding, "hex"))
        .pr$Expr$bin_decode_hex(self, strict)
    else if (identical(encoding, "base64"))
        .pr$Expr$bin_decode_base64(self, strict)
    else
        stop("encoding must be one of {{'hex', 'base64'}}", call. = FALSE)
}
