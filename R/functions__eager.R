...
as_pl_duration = function(x) {
  pcase(
    ...
    # add more types here
    or_else = stop("unknown conversion into pl_duration from: %s", str_string(x))
  )
}
...
difftime_to_pl_duration = function(dft) {
  ...
  u = attr(dft, "units")
  unit = pcase(
    !is_string(u), stop("difftime units should be a string not a: %s", str_string(u)),
    u == "secs", "s",
    u == "mins", "m",
    u == "hours", "h",
    u == "days", "d",
    u == "weeks", "w",
    u == "years", "y",
    or_else = stop("unknown difftime units: %s", u)
  )
  paste0(value, unit)
}
