...
#' @param by `r lifecycle::badge("deprecated")` Something can be coerced to a list of
#'   [expressions][polars_expr]. Used to partition by. Use the `key` parameter instead.
#' @param include_key A bool indicating whether to include the key columns in the output files.
#' @param per_partition_sort_by `r lifecycle::badge("deprecated")` Something can be coerced
#'   to a list of [expressions][polars_expr], or `NULL` (default).
#'   Used to sort over within each partition.
#'   Note that this might increase the memory consumption needed for each partition.
#' @param max_size `r lifecycle::badge("deprecated")` An integer-ish value indicating
#'   the maximum size in rows of each of the generated files.
#'   Use the `max_rows_per_file` parameter instead.
...
constructor = function(
    ...
    if (is.null(key) && !is.null(include_key)) {
      abort("cannot use `include_key` without specifying `key`")
    }
    ...
    new_object(
      ...
      include_key = if (!is.null(key)) include_key %||% TRUE else NULL,
  ...
  constructor = function(
    ...
    deprecate_warn(
      format_warning(
        c(
          `!` = sprintf(
            "%s is deprecated as of %s 1.7.0.",
            format_cls("PartitionMaxSize"),
            format_pkg("polars")
          ),
          i = sprintf("Use %s instead.", format_cls("PartitionBy"))
        )
      )
    )
