...
- Supports Rust 1.65 (#262, #280)
  ...
  - `opt-level` of `argminmax` is now set to `1` in the `release` profile for to support Rust < 1.66.
    The profile can be changed by setting the environment variable `RPOLARS_PROFILE` and when set it to `release-optimized`,
