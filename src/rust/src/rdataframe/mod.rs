...
impl RPolarsDataFrame {
    ...
    pub fn to_list(&self, bigint_conversion: &str) -> List {
    ...
    pub fn to_list_unwind(&self, bigint_conversion: &str) -> Robj {
        let robj_vec_res: Result<Vec<Robj>, _> = collect_hinted_result(
            self.0.width(),
            self.0
                .iter()
    ...
    pub fn to_list_tag_structs(&self, bigint_conversion: &str) -> List {
        //convert DataFrame to Result of to R vectors, error if DataType is not supported
        let robj_vec_res: Result<Vec<Robj>, _> = collect_hinted_result(
            self.0.width(),
            self.0
                .iter()
                .map(|x| pl_series_to_list(x, true, bigint_conversion)),
        );
...
impl RPolarsDataFrame {
    pub fn to_list_result(&self, bigint_conversion: &str) -> Result<Robj, pl::PolarsError> {
        //convert DataFrame to Result of to R vectors, error if DataType is not supported
        let robj_vec_res: Result<Vec<Robj>, _> = self
            .0
            .iter()
            .map(|s| pl_series_to_list(s, true, bigint_conversion))
            .collect();
