...
<!-- badges: start -->
[![R-universe status badge](https://rpolars.r-universe.dev/badges/rpolars)](https://rpolars.r-universe.dev)
...
[polars](https://www.pola.rs/) data manipulation library to R. The underlying
computation engine is written in Rust and this R implementation has no other
dependencies than R itself (≥ 4.1.0).

Documentation can be found on the **rpolars** [homepage](https://rpolars.github.io.html).
...
Søren Welling ([@sorhawell](https://github.com/sorhawell)),
...
100% translation coverage of the underlying "lazy" Expr syntax. While not all of
the "eager"" syntax has yet been translated, you should be able to do just about
everything using `$select()` + `$with_columns()`.
...
pre-compiled **rpolars** binaries for Windows and MacOS (x86_64), with source
builds for other platforms. Please see the GitHub release option below for
binary install options on Linux.
...
install the latest release of **rpolars** on Linux (x86_64) one would use:
...
"Silicon" chips. To install **rpolars** on one of these machines, we need to
...
The introductory vignette (`vignette("rpolars")`) contains a series of detailed
examples. But here is a quick illustration of **rpolars** in action.

Start by loading the package and creating a Polars `DataFrame` object. Similar
to the Python implementation, note that we use the `pl$` prefix to specify a
Polars constructor.
...
Once our Polars DataFrame has been created, we can chain together a series of 
data manipulations as part of the same query. For example:
...
The above is an example of Polars' eager execution engine. But for maximum
performance, it is preferable to use Polars' lazy execution mode, which allows
the package to apply additional query optimizations.
...
[cosine expression](https://rpolars.github.io/reference/Expr_cos.html):
 ...
 - Run `extendr::document()` to recompile and confirm the added method functions as intended, e.g. `pl$DataFrame(a=c(0,pi/2,pi,NA_real_))$select(pl$col("a")$cos())`
 - Run `devtools::test()`. See below for how to set up your development environment correctly.
 
Note that PRs to **rpolars** will be automatically be built and tested on all
...
**Step 5:** Commit your changes and submit a PR to the main **rpolars** repo. 
...
- The `RPOLARS_RUST_SOURCE` environment variable allows **rpolars** to recover the Cargo cache even if source files have been moved. Replace with your own absolute path to your local clone!
