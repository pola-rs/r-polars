...
git_remote_url <- "https://github.com/pola-rs/r-polars/"
...
latest_released_lib_version <- gert::git_remote_ls(remote = git_remote_url) |>
  dplyr::pull(ref) |>
...
desc::desc_set(
  paste0("Config/", package_name, "/lib-version"),
  current_lib_version,
  normalize = TRUE
) |>
  invisible()
